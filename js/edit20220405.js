//2022.04.05
//# 그리기, 저장,  초기화, 취소, 끝점 이어주기, undo(좀 수정필요하긴함)? 이정돈가
var clickPos = [];
var startPos, endPos, undoSavePOs;
var checkFirstPos = true;

var btn_list =[
    {no:0, id:"polygon", txt:'다각형'},
    {no:1, id:"rect", txt:'사각형'},
    {no:2, id:"circle", txt:'원'},
    {no:3, id:"position", txt: '위치'},
    {id:"revise", txt:'수정'},
    {id:"remove", txt:'삭제'},
]
var event_btn_list =[
...
function drawing(e) {
    ...
    if (before_type == "polygon" && endPos != undefined && startPos != undefined) {
...
function undos() {
    if (hist_li.childNodes.length - 1 < 0) {
        ...
        var d_objName = hist_li.lastChild.innerText;
        var r_svg = document.getElementById(String.format("svg_{0}", d_objName));
        hist_li.removeChild(hist_li.lastChild);
        r_svg.children[1].removeChild(r_svg.children[1].lastChild);
    }

    if (now_type == "polygon") endPos = undoSavePOs;
}
